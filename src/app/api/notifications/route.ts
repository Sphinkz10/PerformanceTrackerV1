...
import type { Notification, NotificationListResponse, CreateNotificationPayload, NotificationQuery } from '@/types/notifications';
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Order by
    const orderByClause = query.sortBy === 'priority' ? `CASE priority WHEN 'urgent' THEN 1 WHEN 'high' THEN 2 WHEN 'normal' THEN 3 ELSE 4 END ${query.sortOrder}, created_at DESC` : `created_at ${query.sortOrder}`;

    // Pagination
    const offset = ((query.page || 1) - 1) * (query.limit || 20);

    // Get total count
    const countResult = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/notifications?select=count`, {
      headers: {
    ...
    // Get unread count
    const unreadResult = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/notifications?select=count&read=eq.false&${whereClause}`, {
      headers: {
        apikey: process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || '',
        Authorization: `Bearer ${process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY}`
      }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validCategories = ['pain', 'session', 'form', 'athlete', 'calendar', 'decision', 'system', 'metric', 'injury', 'record'];
