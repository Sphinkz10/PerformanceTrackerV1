...
export async function GET(request: NextRequest, context: {
  ...
  try {
    ...
    const {
      data: links,
      error
    } = await supabase.from('form_field_metric_links').select(`
        *,
        metrics!inner(
          id,
          name,
      ...
      `).eq('form_id', formId).order('created_at', {
      ascending: false
    });
    if (error) {
      console.error('Error fetching metric links:', error);
      return NextResponse.json({
        error: 'Failed to fetch metric links',
  ...
  try {
    ...
    if (auto_create_metric && !metric_id) {
      ...
      if (metricError) {
        ...
        return NextResponse.json({
          error: 'Failed to auto-create metric',
    ...
    const {
      data: link,
      error: linkError
    } = await supabase.from('form_field_metric_links').insert({
      form_id: formId,
      field_id,
      field_name,
    ...
    if (linkError) {
      ...
      return NextResponse.json({
        error: 'Failed to create metric link',
