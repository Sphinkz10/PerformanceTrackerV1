...
export async function POST(request: NextRequest, context: {
  ...
  try {
    ...
    const {
      data: session,
      error: sessionError
    } = await supabase.from('sessions').select('*, workout:workouts(*)').eq('id', sessionId).single();
    ...
    for (const athleteSnapshot of snapshot.athletes || []) {
      ...
      const avgRPE = rpeValues.length > 0 ? rpeValues.reduce((sum, rpe) => sum + rpe, 0) / rpeValues.length : null;
      ...
      for (const pr of prsAchieved) {
        ...
        } = await supabase.from('personal_records').select('value').eq('athlete_id', athleteId).eq('metric_name', metricName).eq('status', 'active').order('value', {
          ascending: false
        }).limit(1);
        ...
        if (isPRVerified) {
          const {
            ...
            category: 'strength',
            // TODO: derive from exercise type
            value: pr.value,
            unit: pr.unit,
            ...
            improvement_percentage: previousBest > 0 ? Math.round((pr.value - previousBest) / previousBest * 100) : 100
          }).select().single();
          if (prError) {
    ...
    if (metricUpdates.length > 0) {
      const {
        error: muError
      } = await supabase.from('metric_updates').insert(metricUpdates);
      if (muError) {
