...
export async function GET(request: NextRequest, {
  ...
  try {
    ...
    const {
      data: snapshot,
      error
    } = await supabase.from('session_snapshots').select(`
        *,
...
export async function DELETE(request: NextRequest, {
  ...
  try {
    ...
    const {
      error: metricsError
    } = await supabase.from('snapshot_metrics').delete().eq('snapshot_id', id);
    if (metricsError) {
    ...
    const {
      error: deleteError
    } = await supabase.from('session_snapshots').delete().eq('id', id);
    if (deleteError) {
