...
export async function GET(request: NextRequest) {
  try {
    ...
    const {
      data: webhooks,
      error
    } = await supabase.from('webhooks').select('*').eq('workspace_id', workspaceId).order('created_at', {
      ascending: false
    });
    if (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate events
    const validEvents = ['session.started', 'session.completed', 'session.cancelled', 'form.submitted', 'metric.updated', 'record.achieved', 'athlete.created', 'athlete.updated', 'workout.created', 'calendar_event.created'];
