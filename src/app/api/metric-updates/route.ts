...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build query
    let query = supabase.from('metric_updates').select('*', {
      count: 'exact'
    }).eq('workspace_id', workspaceId).order('timestamp', {
      ascending: false
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (value_numeric === undefined && value_text === undefined && value_boolean === undefined && value_json === undefined) {
      return NextResponse.json({
