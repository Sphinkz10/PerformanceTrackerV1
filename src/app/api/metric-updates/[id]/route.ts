...
export async function GET(request: NextRequest, context: {
  ...
  try {
    ...
    const {
      data: update,
      error
    } = await supabase.from('metric_updates').select('*').eq('id', id).single();
    if (error || !update) {
      return NextResponse.json({
        error: 'Metric update not found',
...
export async function PUT(request: NextRequest, context: {
  ...
  try {
    ...
    const {
      data: update,
      error
    } = await supabase.from('metric_updates').update(updateData).eq('id', id).select().single();
    if (error) {
      console.error('Error updating metric update:', error);
      return NextResponse.json({
        error: 'Failed to update metric update',
        details: error.message
      }, {
        status: 500
      });
...
export async function DELETE(request: NextRequest, context: {
  ...
  try {
    ...
    const {
      error
    } = await supabase.from('metric_updates').delete().eq('id', id);
    if (error) {
      console.error('Error deleting metric update:', error);
      return NextResponse.json({
        error: 'Failed to delete metric update',
        details: error.message
      }, {
        status: 500
      });
