...
// HELPER: Check cache
// ============================================================================
async function checkCache(supabase: any, workspaceId: string, cacheKey: string): Promise<any | null> {
  const {
    data: cached
  } = await supabase.from('analytics_cache').select('data, expires_at').eq('workspace_id', workspaceId).eq('cache_key', cacheKey).maybeSingle();
  if (cached && new Date(cached.expires_at) > new Date()) {
    // Update hit count
    await supabase.from('analytics_cache').update({
      hit_count: supabase.raw('hit_count + 1'),
      last_accessed_at: new Date().toISOString()
    }).eq('workspace_id', workspaceId).eq('cache_key', cacheKey);
    return cached.data;
  }
  return null;
}
...
// ============================================================================
async function saveCache(supabase: any, workspaceId: string, cacheKey: string, queryType: string, data: any, parameters: any, ttlMinutes: number = 15): Promise<void> {
...
export async function GET(request: NextRequest) {
  try {
    ...
    } = getDateRange(dateRangeParam, startDate, endDate);
    ...
    let sessionsQuery = supabase.from('sessions').select('*', {
      count: 'exact',
      head: true
    }).eq('workspace_id', workspaceId).gte('started_at', start.toISOString()).lte('started_at', end.toISOString());
    const {
      count: totalSessions
    } = await sessionsQuery;
    ...
    } = await supabase.from('session_athletes').select('athlete_id').gte('created_at', start.toISOString()).lte('created_at', end.toISOString());
    ...
    const {
      data: exerciseData
    } = await supabase.from('session_exercise_data').select('data').gte('created_at', start.toISOString()).lte('created_at', end.toISOString());
    let totalVolume = 0;
    ...
    const {
      data: attendanceData
    } = await supabase.from('session_athletes').select('attendance').gte('created_at', start.toISOString()).lte('created_at', end.toISOString());
    const presentCount = (attendanceData || []).filter((sa: any) => sa.attendance === 'present').length;
    const avgAttendance = attendanceData && attendanceData.length > 0 ? Math.round(presentCount / attendanceData.length * 100) : 0;
    ...
    const {
      data: sessions
    } = await supabase.from('sessions').select('started_at').eq('workspace_id', workspaceId).gte('started_at', start.toISOString()).lte('started_at', end.toISOString()).order('started_at', {
      ascending: true
    });
    ...
    const {
    ...
    } = await supabase.from('session_athletes').select(`
        athlete_id,
        athletes (
          id,
          name,
      ...
      `).eq('attendance', 'present').gte('created_at', start.toISOString()).lte('created_at', end.toISOString());
    ...
    const topPerformers = Array.from(athleteSessionCounts.values()).sort((a, b) => b.count - a.count).slice(0, 10).map(item => ({
    ...
    const {
      data: upcomingEvents
    } = await supabase.from('calendar_events').select('id, title, start_date, athlete_ids').eq('workspace_id', workspaceId).eq('status', 'scheduled').gte('start_date', new Date().toISOString()).lte('start_date', new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString()).order('start_date', {
      ascending: true
    }).limit(10);
    ...
    const {
      ...
      `).eq('workspace_id', workspaceId).order('started_at', {
      ascending: false
    }).limit(10);
    ...
    const response = {
      ...
      trends: {
        sessionsOverTime,
        volumeOverTime: [],
        // Could calculate from session_exercise_data
    ...
    // Save to cache
    await saveCache(supabase, workspaceId, cacheKey, 'dashboard', response, {
