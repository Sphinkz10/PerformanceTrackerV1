...
export async function POST(request: NextRequest, {
  ...
  try {
    ...
    const {
      data: event,
      error: eventError
    } = await supabase.from('calendar_events').select('workspace_id, title, start_date').eq('id', eventId).single();
    if (eventError || !event) {
      return NextResponse.json({
    ...
    let query = supabase.from('event_participants').select('id, athlete_id').eq('event_id', eventId);
    ...
    const {
      data: updated,
      error: updateError
    } = await supabase.from('event_participants').update(updateData).in('id', participantIds).select(`
        id,
        athlete_id,
        attendance_status,
