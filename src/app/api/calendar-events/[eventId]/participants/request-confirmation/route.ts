...
export async function POST(request: NextRequest, {
  ...
  try {
    ...
    const {
      data: event,
      error: eventError
    } = await supabase.from('calendar_events').select('workspace_id, title, start_date, end_date, location, type').eq('id', eventId).single();
    if (eventError || !event) {
      return NextResponse.json({
    ...
    // TODO: Send notification/email to each participant
