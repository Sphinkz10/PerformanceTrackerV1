...
export async function PUT(request: NextRequest) {
  try {
    ...
    const {
      data: event,
      error: fetchError
    } = await supabase.from('calendar_events').select('id, recurrence_parent_id').eq('id', eventId).single();
    if (fetchError || !event) {
      return NextResponse.json({
    ...
    const allowedFields = ['title', 'description', 'type', 'status', 'workoutId', 'coachId', 'athleteIds', 'location', 'notes', 'color', 'tags', 'metadata'];
    ...
    } = await supabase.from('calendar_events').update(filteredData).eq('id', parentId);
    ...
    const {
      error: instancesUpdateError,
      count
    } = await supabase.from('calendar_events').update(filteredData).eq('recurrence_parent_id', parentId);
    if (instancesUpdateError) {
      console.error('Error updating instance events:', instancesUpdateError);
      return NextResponse.json({
        error: 'Failed to update instance events',
        details: instancesUpdateError.message
      }, {
        status: 500
      });
    }
    ...
    if (updateData.athleteIds) {
      ...
      const {
        data: seriesEvents
      } = await supabase.from('calendar_events').select('id').or(`id.eq.${parentId},recurrence_parent_id.eq.${parentId}`);
      if (seriesEvents && seriesEvents.length > 0) {
        ...
        const newParticipants = eventIds.flatMap(eventId => updateData.athleteIds.map((athleteId: string) => ({
          event_id: eventId,
          athlete_id: athleteId,
          status: 'pending'
        })));
        await supabase.from('event_participants').insert(newParticipants);
      }
    }
    return NextResponse.json({
      success: true,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const {
      data: event,
      error: fetchError
    } = await supabase.from('calendar_events').select('id, recurrence_parent_id, title').eq('id', eventId).single();
    if (fetchError || !event) {
      return NextResponse.json({
        error: 'Event not found'
      }, {
    ...
    const {
      count: instanceCount
    } = await supabase.from('calendar_events').select('id', {
      count: 'exact',
      head: true
    ...
    const {
      error: instancesDeleteError
    } = await supabase.from('calendar_events').delete().eq('recurrence_parent_id', parentId);
    if (instancesDeleteError) {
      console.error('Error deleting instance events:', instancesDeleteError);
      return NextResponse.json({
        error: 'Failed to delete instance events',
        details: instancesDeleteError.message
      }, {
        status: 500
      });
    }
