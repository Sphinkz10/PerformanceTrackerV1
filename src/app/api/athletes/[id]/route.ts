...
export async function GET(request: NextRequest, context: {
  ...
  try {
    ...
    const {
      data: athlete,
      error
    } = await supabase.from('athletes').select('*').eq('id', athleteId).single();
...
// ============================================================================
async function calculateAthleteStats(supabase: any, athleteId: string): Promise<AthleteStats> {
  ...
  const {
    ...
    count: 'exact',
    head: true
  }).eq('athlete_id', athleteId).gte('created_at', startOfMonth.toISOString());

  // Query 3: Personal records
  const {
    count: personalRecords
  } = await supabase.from('personal_records').select('*', {
    count: 'exact',
    head: true
  ...
  return {
    ...
    // TODO: Calculate from sessions
    avgAttendanceRate: 0,
    // TODO: Calculate from calendar
    totalTrainingHours: 0,
    // TODO: Sum from sessions
