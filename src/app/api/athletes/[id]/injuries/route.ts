...
const MOCK_ATHLETE_INJURIES: Record<string, any[]> = {
  'athlete-1': [{
    ...
    treatment_plan: ['Repouso relativo - evitar impactos', 'Fisioterapia 3x/semana', 'Exercícios excêntricos', 'Gelo após treinos', 'Anti-inflamatórios conforme prescrição'],
    restrictions: ['Sem box jumps', 'Sem double-unders', 'Limitar agachamentos profundos', 'Volume reduzido em 40%'],
    progress_notes: [{
      date: '2024-03-01',
    ...
    treatment_plan: ['Repouso 1 semana', 'Massagem desportiva', 'Exercícios de fortalecimento progressivo', 'Retorno gradual à atividade'],
    restrictions: ['Sem corrida por 1 semana', 'Sem levantamentos pesados por 2 semanas'],
    progress_notes: [{
      date: '2023-11-20',
    ...
    treatment_plan: ['RICE protocol', 'Propriocepção', 'Fortalecimento de tornozelo'],
    restrictions: ['Sem pliometria por 2 semanas'],
    progress_notes: [{
      date: '2023-06-18',
  ...
  'athlete-2': [{
    ...
    treatment_plan: ['Reduzir volume de levantamentos', 'Trabalho de core stability', 'Alongamentos específicos', 'Avaliação postural'],
    restrictions: ['Reduzir carga em 30%', 'Evitar extensão lombar excessiva'],
    progress_notes: [{
      date: '2024-03-15',
...
export async function GET(request: NextRequest, {
  ...
  try {
    ...
    const {
      data: injuries,
      error
    } = await supabase.from('athlete_injuries').select(`
        id,
        athlete_id,
        type,
        body_part,
        ...
        created_at,
        updated_at
      `).eq('athlete_id', athleteId).order('injury_date', {
      ascending: false
    }).limit(limit);
    ...
    if (!error && injuries && injuries.length > 0) {
      ...
      const stats = {
        ...
        avg_recovery_days: injuries.filter(i => i.actual_recovery_date).map(i => {
        ...
        injury_free_days: injuries.length > 0 ? Math.floor((new Date().getTime() - new Date(injuries[0].injury_date).getTime()) / (1000 * 60 * 60 * 24)) : 0
      };
      return NextResponse.json({
    ...
    const stats = {
      ...
      recovering_count: groupedInjuries.recovering.length,
      recovered_count: groupedInjuries.recovered.length,
      avg_recovery_days: allInjuries.filter(i => i.actual_recovery_date).map(i => {
        const start = new Date(i.injury_date);
        const end = new Date(i.actual_recovery_date);
        return Math.floor((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24));
      }).reduce((sum, days, _, arr) => sum + days / arr.length, 0) || 0,
      most_common_area: 'Joelho',
      // Mock
      injury_free_days: allInjuries.length > 0 ? Math.floor((new Date().getTime() - new Date(allInjuries[0].injury_date).getTime()) / (1000 * 60 * 60 * 24)) : 0
    };
    return NextResponse.json({
