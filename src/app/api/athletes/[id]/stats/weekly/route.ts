...
export async function GET(request: Request, context: {
  ...
  try {
    ...
    const {
      data: metrics
    } = await supabase.from('metric_updates').select(`
        metric_id,
        value_numeric,
        timestamp,
        metrics!inner (
      ...
      `).eq('athlete_id', athleteId).gte('timestamp', periodStart).lte('timestamp', periodEnd).order('timestamp', {
      ascending: false
    });

    // Calculate readiness score (average of key biological metrics)
    const readinessMetrics = ['hrv', 'sleep_quality', 'readiness', 'recovery'];
    const relevantMetrics = (metrics || []).filter((m: any) => readinessMetrics.some(rm => m.metrics.name?.toLowerCase().includes(rm)));
    ...
    const {
      ...
      `).eq('session_athletes.athlete_id', athleteId).gte('scheduled_date', weekAgo.toISOString().split('T')[0]).lte('scheduled_date', now.toISOString().split('T')[0]).order('scheduled_date', {
      ascending: false
    });
    ...
    const avg_rpe = sessionsWithRPE.length > 0 ? sessionsWithRPE.reduce((sum: number, s: any) => sum + (s.session_athletes[0]?.avg_rpe || 0), 0) / sessionsWithRPE.length : 0;
    ...
    } = await supabase.from('calendar_events').select('id, status').eq('athlete_id', athleteId).eq('event_type', 'training').gte('start_date', weekAgo.toISOString().split('T')[0]).lte('start_date', now.toISOString().split('T')[0]);
    ...
    const avgVolumePerSession = completedSessions.length > 0 ? weekly_load_actual / completedSessions.length : 500; // Default estimate
    const weekly_load_planned = Math.round(scheduled * avgVolumePerSession);
    const loadPercentage = weekly_load_planned > 0 ? Math.round(weekly_load_actual / weekly_load_planned * 100) : 0;

    // Determine load trend
    let loadTrend: 'up' | 'down' | 'stable' = 'stable';
    if (loadPercentage > 110) loadTrend = 'up';else if (loadPercentage < 90) loadTrend = 'down';
    ...
    const {
      ...
      `).eq('session_athletes.athlete_id', athleteId).gte('scheduled_date', now.toISOString().split('T')[0]).eq('status', 'scheduled').order('scheduled_date', {
      ascending: true
    }).limit(1);
    ...
    const {
      data: alerts
    } = await supabase.from('decisions').select('id', {
      count: 'exact',
      head: true
    }).eq('athlete_id', athleteId).eq('status', 'pending').gte('priority', 100); // Only high priority alerts
    ...
    return NextResponse.json({
      ...
      status: 200
    } // Still return 200 to avoid breaking UI
    );
  }
}
