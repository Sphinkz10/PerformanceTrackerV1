 ...
 * 
 * NOTE: Works standalone with baseline values passed in
 */
...
// ============================================
// CALCULATE THRESHOLD (PURE FUNCTION VERSION)
// ============================================
 ...
 * @param baseline - Baseline value (pass 0 for absolute operators)
...
export function calculateThreshold(
  ...
  try {
    ...
    // If baseline operator but no baseline, try fallback
    if (isBaselineOperator && baseline === 0 && condition.absoluteValue) {
      baseline = condition.absoluteValue;
    }
    
    // Still no baseline? Can't calculate
    if (isBaselineOperator && baseline === 0) {
      console.warn('[calculateThreshold] No baseline available for baseline operator');
      return null;
    }
...
// ============================================
// EVALUATE CONDITION (PURE FUNCTION)
// ============================================
...
/**
 * Calculate thresholds for multiple conditions
 * 
 * @param conditions - Array of rule conditions
 * @param metricValues - Map of metricId -> current value
 * @param baselines - Map of metricId -> baseline value
...
): Array<ThresholdResult | null> {
  return conditions.map((condition) => {
    const currentValue = metricValues[condition.metricId];
    const baseline = baselines[condition.metricId] || 0;
    ...
    return calculateThreshold(condition, baseline, currentValue);
  });
}
