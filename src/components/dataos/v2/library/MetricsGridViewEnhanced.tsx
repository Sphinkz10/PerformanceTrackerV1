 ...
 * ✅ Optimized for touch (swipe gestures ready)
...
interface MetricsGridViewEnhancedProps {
  metrics: Metric[];
...
export function MetricsGridViewEnhanced({
  metrics,
  onMetricClick,
  onEdit,
  onDelete,
      ...
      {metrics.map((metric, index) => {
        ...
        return (
          ...
          >
            ...
            {metric.orphaned && (
              <div className="absolute top-2 right-2 px-2 py-1 rounded-full bg-amber-100 border border-amber-200 text-xs font-semibold text-amber-700">
                Órfã
              </div>
            )}
            ...
            {/* 🎨 STATUS INDICATOR */}
            {metric.status && (
              <div className={`absolute bottom-2 left-2 h-2 w-2 rounded-full ${
                metric.status === 'active' ? 'bg-emerald-500' :
                metric.status === 'inactive' ? 'bg-slate-400' :
                'bg-amber-500'
              }`} />
            )}
          </motion.div>
